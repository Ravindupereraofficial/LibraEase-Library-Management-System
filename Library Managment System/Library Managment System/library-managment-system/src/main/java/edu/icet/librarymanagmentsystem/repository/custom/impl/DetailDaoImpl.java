...
public class DetailDaoImpl implements DetailDao {
    ...
    public Integer getCustomerCount() {
        String query="SELECT COUNT(*) FROM Librarymembers";
    ...
    public Double getFineCount() {
        String query="SELECT SUM(FineAmount) FROM fines";
        try (Connection connection=DBConnection.getInstance().getConnection();
             PreparedStatement preparedStatement=connection.prepareStatement(query);
             ResultSet resultSet=preparedStatement.executeQuery()) {
    ...
    public Integer getOverDueBooksCount() {
        String query="SELECT COUNT(*) FROM borrowrecords WHERE ReturnDate<CURDATE() AND isReturned=0";
        try (Connection connection=DBConnection.getInstance().getConnection();
             PreparedStatement preparedStatement=connection.prepareStatement(query);
             ResultSet resultSet=preparedStatement.executeQuery()) {
            if(resultSet.next()) return resultSet.getInt(1);
    ...
    public Integer getUnpaidFineCount() {
        String query="SELECT COUNT(*) FROM borrowrecords WHERE DateGiven IS NULL";
        try (Connection connection=DBConnection.getInstance().getConnection();
             PreparedStatement preparedStatement=connection.prepareStatement(query);
             ResultSet resultSet=preparedStatement.executeQuery()) {
            if(resultSet.next()) return resultSet.getInt(1);
